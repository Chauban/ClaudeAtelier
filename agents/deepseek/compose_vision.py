...
CRITIQUE_ASK = (
    ...
    "**`ok` 的标准是「这张卡可以挂到墙上去」，不是「这张卡完美」。**\n"
    "  · 只要没有读者一眼就会注意到的毛病，就判 ok=true —— 缺字、被盖住、"
    "溢出画布、错别字、字小到读不出、风格完全不像，这些才是。\n"
    "  · 你仍然可以在 problems 里列出还能更好的地方，那不影响 ok=true，"
    "会连同判词一起存档。\n"
    "  · **在这个缩略尺寸下看不真切的东西，不要当成问题**（字距差半个空格、"
    "边缘一两像素的毛边之类）—— 你看不准的事，报了也是猜。\n"
    "  · 每多返工一轮都有实实在在的成本，而卡片是给人看的，不是给放大镜看的。\n\n"
